...
<form action="managerLogin.php" method="post">
   ...
   <h2 style="margin-left:150px; margin-bottom:30px">Manager Login</h2>
...
<?php
	...
	if(isset($_POST["submit"])) {
		...
		$selectAccountant = "SELECT EMAIL_ID,PASSWORD FROM EMPLOYEE2 NATURAL JOIN MANAGER2 WHERE EMP_NID = 201414004";
		...
		while($row = oci_fetch_assoc($resultAccountant)) {
			if($e_id == $row["EMAIL_ID"] && $pass == $row["PASSWORD"]) {
				$_SESSION["flag"] = "manager";
				$_SESSION["man_e_id"] = $e_id;
