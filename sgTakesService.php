...
<form action="sgTakesService.php" method="post">
  
   <h2 style="margin-left:150px; margin-bottom:30px">Security Guard Service</h2>
...
CONSTRAINT FKWorker1 FOREIGN KEY (Worker_NID) REFERENCES Worker
		...
		$insertTakes_Service .= "VALUES (TO_DATE('{$service_d}','dd/mm/yyyy'),'Security Guard','{$_SESSION['cust_id']}','{$w_nid}',";
...
<?php
	...
	$querySelect .= "WORKING_HOURS,PHONE_NUMBER FROM WORKER NATURAL JOIN SECURITY_GUARD ";
...
<?php
		...
		echo "<strong>License: </strong>".$row["WORKING_HOURS"]."<br/>";
